...
Data encryption-at-rest in the Oracle Cloud Infrastructure (OCI) has been greatly addressed at all the levels. Data encryption-at-rest used in a plethora of security solutions and options, provides data encryption that prevents unauthorized users from accessing data. Any such data leak can result is financial penalties, and or criminal lawsuits that can run ruin company’s reputation.

OCI security initiative implements “ubiquitous encryption” program with the aim to encrypt all data, everywhere. When not specified, the Block Volumes and Object Storage services enables at-rest data encryption by default, using Oracle-Managed encryption keys, by using the Advanced Encryption Standard (AES) algorithm with 256-bit encryption. However, there might be situations where customers are required to manage their own keys for greater compliance or governance reason.

OCI empowers its customers to use their own encryption keys for data encryption. The same keys can be used in OKE to dynamically provision a Storage Volumes which are encrypted using the customer managed keys. This relieves customers re-encrypting the dynamically allocated storage volumes with customer managed keys. The purpose of this blog is to layout steps in detail required to allow Oracle Kubernetes Engine (OKE) to dynamically provision the block volumes, as requested by the application, using data encryption with customer managed-keys in OCI vault.
...
3. Select the desired compartment (*root* unless you've created a compartment specifically for this workshop) and name your vault.
    ...
    All vault types ensure the security and integrity of the encryption keys and secrets that vaults store. A virtual private vault is an isolated partition on a hardware security module (HSM). Vaults otherwise share partitions on the HSM with other vaults. Virtual Private Vault comes with a higher price tag, therefore I will skip this option for the demo.

    Once the Vault is successfully created, it's time to create customer-managed encryption keys for data encryption. Create a key as follows, the customers have an option of importing their key into the OCI Vault as well.

    ---

    </details>

4. Once the Vault has successfully been created, click the name of the fault. Within the **Master Encryption Keys** area, click **`Create Key`**
...
OKE by default create two Storage Classes for dynamic volume provisioning. However, both classes use the Oracle-Managed keys for storage encryption which is the default in the OCI ecosystem.
...
11. Now check the status of the Block Volume to see what was created. Navigate via the main menu to **Storage** -> **Block Volumes**. Click the name of the block volumeto view details.

    ![block volume details](images/view-block-volume.png)

    >Note: Notice that the Encryption key used to encrypt the block volunes i the same that was specified earlier when creating the storage class.
...
13. Open the boot volume, click the `Assign` link next to **Encryption key** property.
...
Oracle Kubernetes Engine (OKE) is highly flexible and allows the users to encrypt the dynamically provisioning volumes with their own keys. Any key rotation operations performed on the Master Encryption Key do not require the modification of the Storage Class already created.
