...
In the previous lab you enabled your application pods to automatically scale out when the existing capacity becomes insufficient. What happens when additional pods are required, but the underlying compute capacity is insufficient? Enter cluster autoscaling!

In this workshop you will enable the number of worker nodes in the node pool to increase automatically when additional pods need to be scheduled. With this configuration, the number of worker nodes can also decrease automatically when the extra capacity is no longer required.
...
* Adds worker nodes to a node pool when a pod cannot be scheduled in the cluster due to insufficient resources.
...
To enable the Kubernetes Cluster Autoscaler to automatically resize a cluster's node pools based on application workload demands, always include resource request limits in pod specifications.
...
2. Make note of the command section towards the end of the above YAML content. There are a few items that will need to be adjusted:
    ...
    * The value of `--scale-down-delay-after-add=5m` was reduced for this workshop. To avoid *thrash* (unnecessarily scaling out and in repeatedly) this would generally be a bit higher. 
...
7. Once you see all 50 pods successfully scheduled, type <kbd>CTRL-c</kbd> to quit. Then type `kubectl get nodes` to check your node pool. Should be an extra one there (just like you saw in the Web UI).
