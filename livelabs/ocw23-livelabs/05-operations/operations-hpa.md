...
Auto scaling is a critical part of managing Kubernetes at scale. By leveraging metrics generated within your cluster, you can define when additional resources are provisioned to support your workloads, reducing the need for more manual work.
...
This is great, but it requires manual intervention to change the number of pods, and that means we need to keep an eye on what's happening. Fortunately for us Kubernetes has support for automating this process based on rules we define.
...
Note that it's also possible to use Prometheus as a data source which will allow you to autoscale on custom metrics. For example, the number of requests to your service. 
...
1. Let's look at how the nodes in the cluster are doing. In the OCI Cloud Shell type:
    ...
    If you get a response `error: metrics not available yet`, it just means that the metrics server is running but hasn't completed it's initial data capture yet. Wait a few seconds and try the `kubectl top nodes` again.
...
5. This is going to run the container with interactive prompt allowing you to execute local commands.  Generate load with the following command:
    ...
    > Note: You cannot leverage the `$EXTERNAL_IP` variable here because the command executes in the BusyBox container, not your Cloud Shell environment.
...
9. In the *new* OCI Cloud Shell (after waiting for a little bit) type:
    ...
    Notice that in this particular example the CPU here for the *mushop-storefront* is at 230m (your number may be different). This is very close to the limit allowed in the storefront `deployment.yaml` file, which has a resource restriction of 300 milli CPU specified. 
...
That we have hit the limit is almost certainly a problem: it's quite likely that the performance of the service is limited out because of this. Of course, you may have made a deliberate decision to limit the service, possibly to avoid overloading the back-end database (though as it's an ATP database it can scale automatically for you if the load gets high).
...
(Normally, of course, this would be handled using modifications to the YAML file for the deployment).
...
1. In the OCI Cloud Shell create an autoscaler:
...
2. In the OCI Cloud Shell type:
    ...
    Of course typing `horizontalpodautoscaler` takes some time, so kubectl has an alias setup for us: we can just type `hpa` instead.
    ...
    The `TARGETS` column tells us what the **current** load is first; this is the average across **all** the pods in the deployment, followed by the target load the autoscaler will aim to achieve by adding or removing pods.

3. Get the autoscaler details. In the OCI Cloud Shell type:
...
8. Let's look at the autoscaler details. In the OCI Cloud Shell type:
    ...
    In fact, it seems that in the time between the commands above the short-term average load increased sufficiently that the autoscaler determined it wanted three pods.
...
9. Let's look at those pods. In the OCI Cloud Shell type:
    ...
    All 3 pods are running and the service is distributing the load amongst them. If the above series of commands were executed in a *very* short period of time, some of the storefront pods may still be in their startup phase.
...
10. Let's get the autoscaler summary again. In the OCI Cloud Shell type:
    ...
    We can see that the average load across the deployment is still over 25%. If it had not reached the maximum number of pods, then the auto scaler would be trying to meet our goal of no more than 25% average load by adding additional pods.

    If you want you can also see the pods being added in the Kubernetes dashboard:

    1. Open the Kubernetes Dashboard.

    2. Make sure you are in the default namespace.

    3. In the left menu under _workloads_ select **Deployments**, then click on the `mushop-storefront` deployment.

    4. In the **Pods** section you can see that, in this case, it's scaled to 3 pods:

        ![Seeing the change in pod count in the dashboard](images/autoscaling-pods-increased.png)

    5. Scroll down to the **Events** section and you can seen the changes it's made:

        ![History of pod count changes in the dashboard](images/autoscaling-dashboard-events.png)

    6. You can see the pod details by opening the replica set:

        ![Details of the auto scaled pods](images/autoscaling-pods-list.png)

    7. Return to the cloud shell window where the load generator is running. Stop the command by typing <kbd>CTRL-c</kbd>. Then type `exit` and press enter to leave the BusyBox container.
        ...
        The autoscaler tries not to "thrash" the system by starting and stopping pods all the time. Because of this it will only remove pods every few minutes rather than immediately the load becomes low. It will also only remove a few pods at a time. The [autoscaler documentation](https://kubernetes.io/docs/tasks/run-application/horizontal-pod-autoscale/#algorithm-details) describes the algorithm.
...
11. In the OCI Cloud Shell type:
    ...
    1. If you would like to force the deployment to scale in, you can type the following in OCI Cloud Shell
...
We've looked at how to use CPU and memory to determine when to autoscale, which may be a good solution, or it may not. Kubernetes autoscaling can support the use of other metrics to manage autoscaling.

These other metrics can be other Kubernetes metrics (known as custom metrics). For example, the number of requests to the ingress controller, or (with the provision of the [Prometheus Adaptor (helm chart)](https://github.com/prometheus-community/helm-charts)) any metric that Prometheus gathers. This last is especially useful as it means you can autoscale on what are effectively business metrics.

It's also possible to autoscale on metrics provided from outside Kubernetes (these are known as external metrics). However, this is only recommended as a last resort due to the security implications, and custom metrics are preferred.
...
We've looked at how to have Kubernetes create new pods based on load (horizontal autoscaling). 

OKE also supports vertical autoscaling (increasing the allowed resource limits on a pod), allowing for situations where a pod may not scale horizontally very well. 

There is another form of autoscaling which can be enabled called _cluster autoscaling_. This monitors the cluster to see if it has any pods that can't be deployed because there are no available workers with enough suitable capacity. It will create new worker nodes as needed, and if there are too many worker nodes for the overall load in the cluster, it will shuffle pods of nodes and stop the node.
